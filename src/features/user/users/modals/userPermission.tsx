...
import { patchSelectedUserPermission, toggleUserPermission } from '../../api'
import { getFormattedPermissions, getPillColor } from '../../helper'
import { loadPermissionDropdownItems, loadSelectedUser, selectLoading, selectPermissionDropdownItems, selectSelectedUser } from '../../store'
import { DropdownItemType, UserResponseType } from '../../type'

const UserPermissions = ({showModal, setShowModal}:{
    showModal:boolean,
    ...
    const permissions:DropdownItemType[] = useSelector(selectPermissionDropdownItems)
    ...
    const togglePermission = async (id: number) => {
        ...
        const response = await toggleUserPermission(selectedPage.id,postData)
    ...
    const grantAllHandle = async (revoke?: boolean) => {
        setErrorMessage('')
        const postData = {
            permissions: revoke? []:permissions.map((item:DropdownItemType )=>item.id)
        }

        const response = await patchSelectedUserPermission(selectedPage.id,postData)
    ...
    return (
        ...
        >
            ...
            <CModalFooter>
                ...
                <Button onClick={()=>grantAllHandle(true)} buttonText={'Revoke All'} buttonType={'danger'} loading={loading}/>
